...
int z;									
int high, wedith;						//定义背景的长宽
int x, y;								//定义坐标点
int gap;								//点之间的间距
int i, j, smallC, middleC, largeC;		//全局变量，不需要重复定义
int gradeCounter = 0;					//记录得分
char showGrade[100];					//用于输出
int size;								//点到能拖动到边界的距离
int num = 0;
int color_size;							//颜色
MOUSEMSG m;								//鼠标操作
COLORREF Defcolor[4];					//颜色


struct Circle							//定义圆的结构体
{
	int x;
	int y;
	int get;
...
struct point							//定义点的结构体
{
	int x;
	int y;
	int getcircle[3];					//获取圆环信息
	int getcolor[3];					//获取颜色信息
};
...
void CleanHeng_1(int clean_color)	//消除第一行
{
	...
	mciSendString("open D:\\design\\xiaochu.mp3 alias xcmusic1", NULL, 0, NULL);
...
void CleanHeng_2(int clean_color)	//消除第二行
{
	...
	mciSendString("open D:\\design\\xiaochu.mp3 alias xcmusic2", NULL, 0, NULL);
...
void CleanHeng_3(int clean_color)	//消除第三行
{
	...
	mciSendString("open D:\\design\\xiaochu.mp3 alias xcmusic3", NULL, 0, NULL);
...
void CleanShu_1(int clean_color)	//消除第一列
{
	...
	mciSendString("open D:\\design\\xiaochu.mp3 alias xcmusic4", NULL, 0, NULL);
...
void CleanShu_2(int clean_color)	//消除第二列
{
	...
	mciSendString("open D:\\design\\xiaochu.mp3 alias xcmusic5", NULL, 0, NULL);
...
void CleanShu_3(int clean_color)	//消除第三列
{
	...
	mciSendString("open D:\\design\\xiaochu.mp3 alias xcmusic6", NULL, 0, NULL);
...
void CleanDouble_type_one(int clean_color)		//消除第一行和第一列
{
	...
	mciSendString("open D:\\design\\xiaochu.mp3 alias music1", NULL, 0, NULL);
...
void CleanDouble_type_two(int clean_color)			//消除第一行和第二列
{
	...
	mciSendString("open D:\\design\\xiaochu.mp3 alias music2", NULL, 0, NULL);
...
void CleanDouble_type_three(int clean_color)	//消除第一行和第三列
{
	...
	mciSendString("open D:\\design\\xiaochu.mp3 alias music3", NULL, 0, NULL);
...
void CleanDouble_type_four(int clean_color)	//消除第二行和第一列
{
	...
	mciSendString("open D:\\design\\xiaochu.mp3 alias music4", NULL, 0, NULL);
...
void CleanDouble_type_five(int clean_color)	//消除第二行和第二列
{
	...
	mciSendString("open D:\\design\\xiaochu.mp3 alias music5", NULL, 0, NULL);
...
void CleanDouble_type_six(int clean_color)	//消除第二行和第三列
{
	...
	mciSendString("open D:\\design\\xiaochu.mp3 alias music6", NULL, 0, NULL);
...
void CleanDouble_type_seven(int clean_color)	//消除第三行和第一列
{
	...
	mciSendString("open D:\\design\\xiaochu.mp3 alias music7", NULL, 0, NULL);
...
void CleanDouble_type_eight(int clean_color)	//消除第三行和第二列
{
	...
	mciSendString("open D:\\design\\xiaochu.mp3 alias music8", NULL, 0, NULL);
...
void CleanDouble_type_nine(int clean_color)	//消除第三行和第三列
{
	...
	mciSendString("open D:\\design\\xiaochu.mp3 alias music9", NULL, 0, NULL);
...
void CleanXie_left(int clean_color)	//消除左上到右下颜色相同的环
{
	...
	mciSendString("open D:\\design\\xiaochu.mp3 alias xcmusic7", NULL, 0, NULL);
...

{
	...
	mciSendString("open D:\\design\\xiaochu.mp3 alias xcmusic8", NULL, 0, NULL);
...
void Clear_FullPoint()	//消除同一点上三个圈颜色相同的环
{
	for (i = 1; i <= 9; i++)
	{
		if ((p[i].getcircle[0] == 1 && p[i].getcircle[1] == 1 && p[i].getcircle[2] == 1) && (p[i].getcolor[0] == 1 && p[i].getcolor[1] == 1 && p[i].getcolor[2] == 1))//同一点三个圆环颜色为红色
		{
			...
			mciSendString("stop xcmusic1", NULL, 0, NULL);   // 先把前面一次的音乐停止
			mciSendString("close xcmusic1", NULL, 0, NULL); // 先把前面一次的音乐关闭
			mciSendString("open D:\\design\\xtxiaochu.mp3 alias xcmusic1", NULL, 0, NULL);
			mciSendString("play xcmusic1", NULL, 0, NULL); // 仅播放一次
...
void starup()	//游戏初始化
{
	...
	loadimage(&img_bk, "D:\\design\\bkg.jpg");	// 读取图片到 img 对象中
...
void Draw_ring(int c, Circle New_c[3])                                 //画圆
{
	for (i = 1; i <= 3; i++)
	{
...
void Draw_pointring(int c, point New_p[3])								//画点
{
	for (i = 1; i <= 9; i++)
	{
...
void show()
{
	mciSendString("open D:\\design\\bkmusic.mp3 alias bkmusic", NULL, 0, NULL);//背景音乐
	...
	setfillcolor(RGB(0, 51, 51));//设置背景颜色
	solidrectangle(0, 0, high, wedith);//画背景
	sprintf(showGrade, "%d", gradeCounter);
	outtextxy(2, 02, showGrade);
	...
	setlinestyle(PS_SOLID, 3);//线条的粗细
	setlinecolor(Defcolor[0]);//环的颜色为红色
	Draw_ring(1, c);
	Draw_pointring(1, p);
	setlinecolor(Defcolor[1]);//环的颜色为蓝色
	Draw_ring(2, c);
	Draw_pointring(2, p);
...
void withoutinput()
{
	...
	for (color_size = 1; color_size <= 4; color_size++)      //消除的检验
	{
...
void withinput()
{
	m = GetMouseMsg();                                                                    //鼠标的具体操作，获取位置信息
	size = 53;
	if (m.uMsg == WM_LBUTTONDOWN && m.x <= 314 && m.x >= 266 && m.y >= 441 && m.y <= 489) //判断鼠标有没有在圆环1内点下
	{
		c[3].get = 1;
	}
	if (m.uMsg == WM_LBUTTONDOWN && m.x <= 124 && m.x >= 76 && m.y >= 441 && m.y <= 489)  //判断鼠标有没有在圆环2内点下
	{
		c[1].get = 1;
	}
	if (m.uMsg == WM_LBUTTONDOWN && m.x <= 219 && m.x >= 171 && m.y >= 441 && m.y <= 489) //判断鼠标有没有在圆环3内点下
	{
		c[2].get = 1;
	}
